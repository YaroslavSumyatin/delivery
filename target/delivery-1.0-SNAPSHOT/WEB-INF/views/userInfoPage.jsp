...
<div class="container">
    ...
    <main class="container_main">
        <div class="userinfo_content">
            ...
            <a id="delete_link" href="${pageContext.request.contextPath}/profile/delete"
               onclick="return check()"><fmt:message key="profile.delete_profile"/></a><br>
                ...
                <c:forEach items="${applications}" var="a">
                    <c:if test="${not (a.state eq Application.STATE_SENT)}">
                        <div class="userprofile_applications-item">
                            ...
                            <fmt:message key="application.state"/>: <fmt:message key="application.state.${a.state}"/>
                            <br>
                            ...
                            <c:if test="${not (a.state eq Application.STATE_SENT)}">
                                <form id="delete_app${a.id}" action="${pageContext.request.contextPath}/profile"
                                      method="post">
                                    <input type="hidden" name="application" value="${a.id}">
                                    <input type="button" value="<fmt:message key="profile.cancel_applications"/>"
                                           onclick="return check(${a.id})"/>
                                </form>
                            </c:if>
                        </div>
                    </c:if>
